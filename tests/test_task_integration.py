...
from student_task_manager.domain.models import Priority, Status, Student
from student_task_manager.services.task_service import TaskService


@pytest.fixture
def service_with_database(monkeypatch: pytest.MonkeyPatch) -> Iterator[TaskService]:
...
def test_integration_create_task_round_trips_through_database(
    ...
    stored_task = service_with_database.get_task_by_id(created_task.id)
...
def test_integration_complete_task_updates_persisted_state(
    ...
    service_with_database.mark_complete(created_task.id)
    stored_task = service_with_database.get_task_by_id(created_task.id)
...
def test_integration_filter_tasks_uses_persisted_records(
    ...
    service_with_database.mark_complete(high_task.id)
